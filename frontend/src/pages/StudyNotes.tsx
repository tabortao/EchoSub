  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
        <Title level={4} style={{ margin: 0, fontSize: isPhone ? 18 : 20 }}>学习页面</Title>
        <Space wrap>
          <Select
            placeholder="按专辑筛选"
            allowClear
            style={{ width: isPhone ? '100%' : 180, minWidth: isPhone ? '100%' : 160 }}
          ...
          <Button
            ...
            style={{ minHeight: 44, width: isPhone ? '100%' : 'auto' }}
          >
            新建学习页面
          </Button>
        </Space>
      ...
      {loading ? (
        ...
        <Row gutter={[16, 16]}>
          {notes.map((n) => (
            <Col xs={12} sm={12} md={8} lg={6} xl={6} xxl={4} key={n.id}>
              <Card
                hoverable
              ...
              >
                <Card.Meta
                  title={<Text ellipsis style={{ maxWidth: '100%', fontSize: isPhone ? 14 : 14 }}>{n.title}</Text>}
                  description={
                    <div>
